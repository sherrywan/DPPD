...
from lib.learning.criterion import choose_criterion
from lib.learning.optimizer import choose_optimizer, choose_scheduler
from lib.learning.utils import AverageMeter, accuracy, save_checkpoint, assert_learning_params
from const import path, const
from lib.utils.utils import is_substring, check_uniformity_and_get_first_elements
...
def final_test(model, test_loader, params):
    ...
    with torch.no_grad():        
        for x, residual_feat_pose, residual_feat_motion, y, video_idx, metadata in loop:
            x, y = x.to(device), y.to(device)
            residual_feat_pose = abs(residual_feat_pose).to(device)
            residual_feat_motion = abs(residual_feat_motion).to(device)
            ...
            if params['medication']:
                ...
                out = model(x, metadata, residual_feat_pose, residual_feat_motion)
...
def validate_model(model, validation_loader, params, class_weights):
    ...
    with torch.no_grad():
        ...
        for x, residual_feat_pose, residual_feat_motion, y, video_idx, metadata in validation_loader:
            x, y = x.to(device), y.to(device)
            residual_feat_pose = abs(residual_feat_pose).to(device)
            residual_feat_motion = abs(residual_feat_motion).to(device)
            metadata = metadata.to(device)
...
def train_model(params, class_weights, train_loader, val_loader, model, fold, backbone_name, mode="RUN"):
    ...
    for epoch in loop:
        ...
        for train_idx, (x, residual_feat_pose, residual_feat_motion, y, video_idx, metadata) in enumerate(train_loader):
            x, y = x.to(device), y.to(device)
            residual_feat_pose = abs(residual_feat_pose).to(device) 
            residual_feat_motion = abs(residual_feat_motion).to(device)
            metadata = metadata.to(device)
        ...
        if val_acc > acc_best:
            acc_best = val_acc
            acc_best_flag = True
            save_checkpoint(checkpoint_root_path, epoch, lr_backbone, optimizer, model, None, fold, latest=False)
            print(f'Checkpoint saved at: {checkpoint_root_path}')
        if acc_best>=100:
            break
            
    if mode == "RUN":
        if not acc_best_flag:
            save_checkpoint(checkpoint_root_path, epoch, lr_backbone, optimizer, model, None, fold, latest=False)
