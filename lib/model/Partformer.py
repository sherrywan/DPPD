...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, st_mode='stage_st', att_fuse=False, part_len=[], blocks_joint2part=None, blocks_part2joint=None):
        super().__init__()
        # assert 'stage' in st_mode
        self.st_mode = st_mode
        self.parts_len = part_len
        ...
        self.blocks_joint2part = blocks_joint2part                          
        self.blocks_part2joint = blocks_part2joint
    ...
    def forward(self, x, seqlen=1):
        ...
        if self.st_mode=='stage_st_part':
            BF,J,C = x.shape
            # inter-part spatical 
            part_outs = []
            p_start = 0
            for p_i, p in enumerate(self.parts_len):
                p_end = p_start + p
                x_part = x[:,p_start:p_end].clone()
                x_part = x_part + self.drop_path(self.attn_s(self.norm1_s(x_part), seqlen))
                x_part = x_part + self.drop_path(self.mlp_s(self.norm2_s(x_part)))
                x_part = x_part.reshape(BF,1,-1)
                x_part = self.blocks_joint2part[p_i](x_part)
                part_outs.append(x_part)
                p_start = p_end
            part_outs = torch.stack(part_outs, dim=1).squeeze(dim=2)
            # intra-part spatical 
            part_outs = part_outs + self.drop_path(self.attn_s(self.norm1_s(part_outs), seqlen))
            part_outs = part_outs + self.drop_path(self.mlp_s(self.norm2_s(part_outs)))
            p_start = 0
            for p_i, p in enumerate(self.parts_len):
                p_end = p_start + p
                x_part = part_outs[:,p_i:(p_i+1)]
                x_part = self.blocks_part2joint[p_i](x_part)
                x_part = x_part.reshape(BF,p,-1)
                x[:,p_start:p_end] = x_part
                p_start = p_end    
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen))
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))
        elif self.st_mode=='stage_ts_part':
            BF,J,C = x.shape
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen))
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))
            # inter-part spatical 
...
class Partformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
                 ...
                 num_joints=17, parts_list={"torse":[0,7,8,9,10], "larm":[11,12,13], "rarm":[14,15,16], "lleg":[4,5,6], "rleg":[1,2,3]}, maxlen=81, 
        ...
        self.blocks_joint2part = nn.ModuleList([
            nn.Linear(dim_feat * j, dim_feat)
            for j in self.parts_len])                           
        self.blocks_part2joint = nn.ModuleList([
            nn.Linear(dim_feat, dim_feat * j)
            for j in self.parts_len])
        self.blocks_st = nn.ModuleList([
                Block(
                    dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                    st_mode="stage_st_part", part_len=self.parts_len, blocks_joint2part = self.blocks_joint2part, blocks_part2joint = self.blocks_part2joint)
                for i in range(depth)])  
        self.blocks_ts = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_ts_part", part_len=self.parts_len, blocks_joint2part = self.blocks_joint2part, blocks_part2joint = self.blocks_part2joint)
            for i in range(depth)])
        self.norm = norm_layer(dim_feat)
        
        if dim_rep:
            self.blocks_pre_logits = nn.ModuleList([
                nn.Sequential(OrderedDict([
                    ('fc', nn.Linear(dim_feat*p, dim_rep)),
                    ('act', nn.Tanh())
                ]))
                for p_i, p in enumerate(self.parts_len)])
        else:
            self.blocks_pre_logits = nn.Identity()
        self.blocks_head = nn.ModuleList([
            nn.Linear(dim_rep, dim_out*p)
            for p_i, p in enumerate(self.parts_len)])
        # self.head = nn.Linear(dim_rep, dim_out) if dim_out > 0 else nn.Identity()            
        self.temp_embed = nn.Parameter(torch.zeros(1, maxlen, 1, dim_feat))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_joints, dim_feat))
        self.part_embed = nn.Parameter(torch.zeros(1, self.num_parts, dim_feat))
    ...
    def forward(self, x, return_rep=False):   
        ...
        # add joint_index embedding
        x = x + self.pos_embed
        
        # add part_index embedding
        p_start = 0
        for p_i, p in enumerate(self.parts_len):
            p_end = p_start + p
            x[:,p_start:p_end] = x[:,p_start:p_end] + self.part_embed[:,p_i:(p_i+1)]
            p_start = p_end
        ...
        for p_i, p in enumerate(self.parts_len):
                p_end = p_start + p
                x_part = x[:,:,p_start:p_end]
                x_part = x_part.reshape(BF,1,-1)
                x_part = self.blocks_pre_logits[p_i](x_part)
                part_rep_outs.append(x_part.reshape(B,F,1,-1))
                x_part = self.blocks_head[p_i](x_part)
                x_part = x_part.reshape(B, F, p, -1)
                part_outs.append(x_part)
                p_start = p_end
        part_rep_outs = torch.stack(part_rep_outs, dim=2).squeeze(dim=3)
        x = torch.cat(part_outs, dim=2)
        if return_rep:
            return part_rep_outs
        else:
            return x
