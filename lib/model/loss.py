...
def supervised_contrastive_loss(features, labels, temperature=0.07):
    """
    features: [N, C] 经过池化的 predicted_feat
    ...
    logits = features @ features.T / temperature  # (N, N)

    # Positive mask: same subject (include self)
    labels = labels.view(-1, 1)
    positive_mask = (labels == labels.T).float()  # self-included!

    # Log-softmax for denominator
    logits_max, _ = logits.max(dim=1, keepdim=True)
    logits = logits - logits_max.detach()
    exp_logits = torch.exp(logits)
    log_prob = logits - torch.log(exp_logits.sum(dim=1, keepdim=True))  # log-softmax

    # Average log prob over all positives (including self)
...
def p_mpjpe(predicted, target):
    ...
    # Return MPJPE
    return predicted_aligned, np.mean(np.linalg.norm(predicted_aligned - target, axis=len(target.shape)-1), axis=1)

def loss_velocity_feat(predicted):
    """
    ...
    return torch.mean(torch.norm(velocity_predicted, dim=-1))
