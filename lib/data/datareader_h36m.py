...
class DataReaderH36M(object):
    def __init__(self, args, n_frames, sample_stride, data_stride_train, data_stride_test, read_confidence=True, dt_root = 'data/Human3.6', dt_file = 'h36m_sh_conf_cam_source_final.pkl',
                 parts_list={"torse":[0,7,8,9,10], "larm":[11,12,13], "rarm":[14,15,16], "lleg":[4,5,6], "rleg":[1,2,3]}, num_joints=17):
    ...
    def read_2d(self):
        ...
        if self.joints_index == "part":
            trainset = trainset[:,self.joint2part_index]
            testset = testset[:,self.joint2part_index]
    ...
    def read_3d(self):
        ...
        if self.joints_index == "part":
            train_labels = train_labels[:,self.joint2part_index]
            test_labels = test_labels[:,self.joint2part_index]    
