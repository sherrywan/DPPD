...
class Augmenter_input(object):
    ...
    def add_noise(self, motion_input):
        batch_size = motion_input.shape[0]
        num_frames = motion_input.shape[1]
        num_joints = motion_input.shape[2]
        mean = float(self.noise['mean'])
        std = float(self.noise['std'])
        delta_noise = torch.randn(num_frames, num_joints, 3) * std + mean
        delta_noise = delta_noise.to(motion_input.device)
        motion_input = motion_input + delta_noise
    ...
    def augment(self, motion_input, mask=False, noise=False):     
        if noise:
            motion_input = self.add_noise(motion_input)
        if mask:
            motion_input = self.add_mask(motion_input)
        return motion_input
